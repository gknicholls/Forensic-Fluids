...
Load the output files from the example classification analyses with BDP into R.
The output file for Bayesian analysis is \texttt{testSamplesBin\_2022\_09\_22\_bayes\_bdp\_J5\_all.log}, while that for the Cut-Model analysis is \texttt{testSamplesBin\_2022\_09\_22\_cut\_bdp\_J5\_all.log}, which are both tab-delimited files.

```{r load_output_files}
# Bayesian analysis with BDP (J_f = 5 & L_g = M_g)
rowJ5ColJMax.df = read.table(file = "testSamplesBin_2022_09_22_bayes_bdp_J5_all.log",
...
rowJ5ColJMaxCut.df = read.table(file = "testSamplesBin_2022_09_22_cut_bdp_J5_all.log",
                                header = T, as.is = T, sep = "\t")
...
The main result of interest is fluid type classification, so in this section, we produce numeric matrices that summaries the posterior distribution across the five fluid types for each unlabelled profile.
These numeric matrices have 46 rows and five columns, respectively corresponding to 46 unlabelled profiles in the test set and the five fluid types considered.
...
This section summarises the posterior distribution of the number of subtypes within each fluid type.
Specifically, the code below created the barplots in Figure 10 panels a) and b).
