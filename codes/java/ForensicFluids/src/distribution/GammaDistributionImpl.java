...
public class GammaDistributionImpl extends AbstractRealDistribution implements Serializable {
     ...
     * @throws MathRuntimeException if the cumulative probability can not be
     *                       computed due to convergence or other numerical errors.
     */
    @Override
    public double cumulativeProbability(double x) throws MathRuntimeException {
        double ret;
     ...
     * @throws MathRuntimeException            if the inverse cumulative probability can not be
     *                                  computed due to convergence or other numerical errors.
    ...
    private void setAlphaInternal(double newAlpha) {
        if (newAlpha <= 0.0) {
            throw new RuntimeException("alpha must be positive ({0}): "+newAlpha);
        }
        this.alpha = newAlpha;
    }
    ...
    private void setBetaInternal(double newBeta) {
        if (newBeta <= 0.0) {
            throw new RuntimeException("beta must be positive ({0}): "+newBeta);
        }
        this.beta = newBeta;
    }
